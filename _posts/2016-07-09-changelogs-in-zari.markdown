...
예약메모의 경우 게스트하우스, 호스텔, 중소규모 호스텔에서 일어나는 정말 다양한 케이스를 정리하는 중요한 곳입니다. 이리저리 고민하다가 결국 순정이 최고라는 결론에 깨닫고 고전적인 `text diff`를 가져왔습니다.

신기한 것은 이걸 잘 활용하는 분들이 많다는 것입니다. 예를 들어 키보증금을 체크아웃 때 환불해주라고 담당자에게 메모를 `키보증금` 남기면 처리하고나서 `(환불)키보증금` 이렇게 적게됩니다. 이제는 (일부) 과감하게 `키보증금`을 지워버립니다. 어차피 빨간줄이 그어지고, 누가했는지 표시가 되니까요.
